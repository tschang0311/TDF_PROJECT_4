...
def play_mp3(file_path: Path, index: int, music_dir: Path) -> None:
"""Play the selected audio file on the computer speakers."""
...
return
# Build the command to play audio
# On macOS, use afplay (built-in audio player)
# On Linux, try aplay or mpg123
# On Windows, use start with /wait
if sys.platform == "darwin":
cmd = ["afplay", str(audio_file)]
elif sys.platform == "linux":
# Try mpg123 first, fall back to aplay
cmd = ["mpg123", str(audio_file)]
elif sys.platform == "win32":
cmd = ["start", "/wait", str(audio_file)]
else:
print(f"Error: Unsupported platform {sys.platform}", file=sys.stderr)
return
print(f"Playing: {audio_file.name} (index {index})", file=sys.stderr, flush=True)
# Launch the audio player process
process = subprocess.Popen(
cmd,
stdout=subprocess.PIPE,
stderr=subprocess.PIPE,
# Don't redirect stdin to allow the process to run independently
)
print(f"Audio playback started (PID: {process.pid})", file=sys.stderr, flush=True)
with playback_lock:
...
print(f"Error playing audio file {file_path.name}: {exc}", file=sys.stderr)
...
play_mp3(file_path, index, directory)
else:
...
def handle_up_button_command() -> None:
"""Handle an UP_BUTTON command from the Arduino."""
print("hello world")
...
elif line.startswith("UP_BUTTON:"):
handle_up_button_command()
