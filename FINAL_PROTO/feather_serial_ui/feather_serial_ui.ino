...
bool lastUpButtonState = true;      // true = not pressed (pullup)
bool lastDownButtonState = true;    // true = not pressed (pullup)
bool lastLeftButtonState = true;    // true = not pressed (pullup)
bool lastRightButtonState = true;   // true = not pressed (pullup)
...
void setup() {
  ...
  tft.setRotation(3);  // horizontal layout (240x135) - flipped 180 degrees
...
void handleButtonPresses() {
  ...
  bool currentUpState = encoder.digitalRead(SS_SWITCH_UP);
  bool currentDownState = encoder.digitalRead(SS_SWITCH_DOWN);
  bool currentLeftState = encoder.digitalRead(SS_SWITCH_LEFT);
  bool currentRightState = encoder.digitalRead(SS_SWITCH_RIGHT);

  unsigned long currentTime = millis();

  // Handle SELECT button press (transition from HIGH to LOW due to pullup)
  if (lastSelectButtonState && !currentSelectState) {
    // Button was just pressed
    if (currentTime - lastSelectButtonPressTime > BUTTON_DEBOUNCE_MS) {
      // Valid button press
      if (currentTracks.empty()) {
        // No tracks available
        return;
      ...
      if (focusedIndex >= 0 && focusedIndex < static_cast<int32_t>(currentTracks.size())) {
        ...
        if (isPlaying(focusedIndex)) {
          ...
          Serial.print("PLAY:");
          Serial.println(focusedIndex);
          // Run graphics test when a file is selected
          runGraphicsTest();
          // Restore the track list display after graphics test
          if (!currentTracks.empty()) {
            updateScrollOffsetFromFocus();
            renderTrackList(currentTracks, ST77XX_GREEN, true, scrollOffset, focusedIndex, playingIndices);
          }
      ...
      lastSelectButtonPressTime = currentTime;
    }
  }

  // Handle UP button press (transition from HIGH to LOW due to pullup)
  if (lastUpButtonState && !currentUpState) {
    // Button was just pressed
    if (currentTime - lastUpButtonPressTime > BUTTON_DEBOUNCE_MS) {
      // Valid button press - send command to Python
      Serial.println("UP_BUTTON:");
      lastUpButtonPressTime = currentTime;
    }
  }

  // Handle DOWN button press (transition from HIGH to LOW due to pullup)
  if (lastDownButtonState && !currentDownState) {
    // Button was just pressed
    if (currentTime - lastDownButtonPressTime > BUTTON_DEBOUNCE_MS) {
      // Valid button press - add your DOWN button logic here
      Serial.println("DOWN button pressed");
      // TODO: Add DOWN button functionality
      lastDownButtonPressTime = currentTime;
    }
  }

  // Handle LEFT button press (transition from HIGH to LOW due to pullup)
  if (lastLeftButtonState && !currentLeftState) {
    // Button was just pressed
    if (currentTime - lastLeftButtonPressTime > BUTTON_DEBOUNCE_MS) {
      // Valid button press - add your LEFT button logic here
      Serial.println("LEFT button pressed");
      // TODO: Add LEFT button functionality
      lastLeftButtonPressTime = currentTime;
    }
  }

  // Handle RIGHT button press (transition from HIGH to LOW due to pullup)
  if (lastRightButtonState && !currentRightState) {
    // Button was just pressed
    if (currentTime - lastRightButtonPressTime > BUTTON_DEBOUNCE_MS) {
      // Valid button press - add your RIGHT button logic here
      Serial.println("RIGHT button pressed");
      // TODO: Add RIGHT button functionality
...
void runGraphicsTest() {
  // Graphics test removed - screen cleared
  tft.fillScreen(ST77XX_BLACK);
}
